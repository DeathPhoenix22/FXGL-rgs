...
class TilesetLoader(private val map: TiledMap, private val mapURL: URL) {
    ...
    fun loadViewStaggered(layerName: String): Node {
        log.debug("Loading staggered view for layer $layerName")
        ...
        // +1 to handle rounding errors due to integer division
        val buffer = WritableImage(
            (layer.width * map.tilewidth) + (map.tilewidth / 2 + 1),
            (layer.height + 1) * map.tileheight / 2 + 1
        )
        ...
        for (i in 0 until layer.data.size) {
            ...
            val staggeredX = if (y % 2 != 0) map.tilewidth / 2 else 0
            val bufferX = x * map.tilewidth + staggeredX - offsetX
            val bufferY = y * map.tileheight / 2 - offsetY
